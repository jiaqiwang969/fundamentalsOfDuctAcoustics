...
f=[6000]
w=5;%2*pi*rT/343;
Tr=0.0;Omag=0.0; Mx=0.5*ones(N+1,1);M_theta=Tr./r+Omag*r;
[c02,rou0,P0,s0]=entropyPara(r,N,Ratio,Omag,Tr,Entropy,beta);
m=[0];
n=[1:6];


% if Boundary==1 && Tr==0 && Omag==0 && Ratio<0.01 && sum(diff(Mx))==0
for nk=1:length(m)
        [Base] = BaseJ1(m,n(end),1);   %Rienstra-50, at Least n-radialModes calculated
        beta=sqrt(1-mean(Mx)^2);
        Eig1=(-w*mean(Mx)-sqrt(w^2-beta^2*Base.jmn_pm.^2))/beta^2; %Rienstra-83
        Eig2=(-w*mean(Mx)+sqrt(w^2-beta^2*Base.jmn_pm.^2))/beta^2; %Rienstra-83
        % U_r{kn,km} = Base.normValue(n(kn),km)*chebfun(@(t) besselj(sign(m(km)).*m(km),Base.jmn_pm(n(kn))*t),[0,rT]);
        % else
        [V,lam]=eigfun_AB(r,D,N,w,m,Ratio,Mx,M_theta,rou0,P0,c02,Boundary,z_t,z_h);%characteristics
